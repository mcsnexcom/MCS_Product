...
int main(int argc, char *argv[]) {
	...
	{
		printf("------------------------------------\n");
		printf("Power button setting example:\n");
		printf("NO ACTION: ./demoPowerButton 0\n");
		printf("NORMAL   : ./demoPowerButton 1\n");
		printf("------------------------------------\n");
        exit(EXIT_FAILURE);
	...
	*/
	...
	/* Set Power Button */
	DataIn[0] = atoi(argv[1]);
	DataInLength = 1;
	res = LAUNCH_MCU_CMD("ST_SYSTEM_SET_50_01_POWER_BUTTON_CONTROL", DataIn, DataInLength, DataOut, &DataOutLength);
	if(res > 0)
	{	
		switch(DataIn[0])
		{
			case 0:
				printf("Set power button to NO ACTION.\n");
				break;
			case 1:
				printf("Set power button to notmal behavior.\n");
				break;
			default:
				break;
		}
		...
		printf("Set power button control failure.\n");

	usleep(100000);
	/* Update setting to flash */
	DataIn[0] = 0x01;
	DataInLength = 1;
	res = LAUNCH_MCU_CMD("ST_SYSTEM_SET_20_01_FLASH_UPDATE", DataIn, DataInLength, DataOut, &DataOutLength);
	if(res > 0)
		printf("Update setting successfully\n");
	else
		printf("Update setting failure !!!\n");

...
\[\!IMPORTANT\]  
Updates are only provided within the same JetPack version. Nexcom OS currently does NOT support cross-version JetPack upgrades (e.g., upgrading from JetPack 5.1.2 to JetPack 6.x).
NOTED: The image file for OTA updates can only be used with Jetpack 6.0 and later.
...
### 1-1. Contact the **Nexcom FAE Team** to obtain:  
...
### 1-3. Unpack the OTA tools:  
    sudo tar \-C $WORKDIR \-xvf ota\_tools\_\<rel\>\_aarch64.tbz2

### 1-4. Create the /ota/ directory and place the payload package inside:  
    sudo mkdir \-p /ota/   
...
### 2-2. Add paths of files or directories you wish to keep (use absolute paths).  Example items already included:  
...
### **4 Update Bootloader (For Orin-NX & Orin-Nano)** 【If using **Orin-NX** or **Orin-Nano** and you need to update UEFI】:
...
### **5 Reboot System** After triggering the update, reboot to begin the installation:
    sudo reboot \-h now
...
### 7-1. **Logs:** Check the directory /last\_ota\_update\_log for detailed success/failure logs.  
### 7-2. **Version Check:** Verify the updated version:  
